...
public:
	...
	inline double compute( double k, tf_type type ){
		float kjeq = 9;//(0.01*cosmo_.H0);
		float x = 1.61*k/kjeq;
		double ftf = cos(pow(x,3))/(1.+pow(x,8));
...
namespace{
	transfer_function_plugin_creator_concrete< transfer_fuzzy_plugin > creator("fuzzy");
}
