...
class transfer_fuzzy_plugin : public transfer_function_plugin
{
protected:
	using transfer_function_plugin::cosmo_; eisenstein_transfer etf_;
public:
transfer_fuzzy_plugin( config_file &cf )//Cosmology aCosm, double Tcmb = 2.726 )
    :  transfer_function_plugin(cf)
	{
	double Tcmb = pcf_->getValueSafe("cosmology","Tcmb",2.726);
	etf_.set_parameters( cosmo_, Tcmb );
...
inline double compute( double k, tf_type type ){
	float fdmmass = 1.0; // FDM mass in unit of 1e-22 eV
	float kjeq = 9.*sqrt(fdmmass)/(0.01*cosmo_.H0);
	//std::cout << cosmo_.H0 << "Hubble";
	float x = 1.61*k/kjeq*pow(fdmmass,1./18.);
	float ftf = cos(pow(x,3))/(1.+pow(x,8));
        return etf_.at_k( k )*ftf;
	}
inline double get_kmin( void ){
		return 1e-4;
	}
...
transfer_function_plugin_creator_concrete< transfer_fuzzy_plugin > creator4("fuzzy");
}
